...
if __name__ == '__main__':
    ...
    percents_df_full = overview_20xx_df[['player', 'serve_pts', 'first_in', 'first_won', 'second_in', 'second_won', 'return_pts', 'return_pts_won', 'winners', 'unforced']].groupby('player').sum().reset_index()
    ...
    percents_df = percents_df_full[['player', 'first_in_percent', 'first_won_percent', 'second_won_percent', 'return_points_won_percent', 'winners_percent', 'unforced_percent']]
    ...
    player_aces_and_unret = serveBasics[['player', 'aces_and_unret']].groupby('player').mean().reset_index()
    print(player_unret)
